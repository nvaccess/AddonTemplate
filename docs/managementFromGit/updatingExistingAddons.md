...
Ensuring your add-on behavior remains consistent during development and following template updates is done through unit testing.

For the unit tests to execute successfully, the target module (such as `syncAddonWithTemplate.py`) must be located at the root of the repository as a sibling file to the `tests/` directory.

They can be validated quickly using `unittest` inside the virtual environment managed by `uv`.
